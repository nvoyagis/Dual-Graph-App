...
def simulate_dual(sims: int, seed: int, stocks: list[str], begin_data_date: str, buy_date: str, sell1: str, sell2: str, analyze_node_features = False, frequently_appearing_check = False):
    ...
    for s in stocks:
        df = pd.read_csv(
                        f'app/Data2015-2025/HistoricalPrices 2015 - 2025, {s}.csv',
                        parse_dates=['Date'],
                        date_format='%m/%d/%y')
        df = df[df['Date'] <= sell2]
    ...
    nx.draw(g, pos=pos0, with_labels=True, node_color='#a0bec8', edge_color='#a0bec8', node_size=500, font_size=12, font_family='STIXGeneral', ax=ax)
    # Draw edge labels
