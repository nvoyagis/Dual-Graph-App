...
def download_with_retry(
    ...
    *,
    ...
    """
    Retry yfinance downloads with exponential backoff.
    ...
    -------
    pd.DataFrame
        Downloaded data. May be empty if all retries fail.
    """
    last_error = None
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            last_error = ValueError("yfinance returned an empty DataFrame")

        except Exception as e:
            last_error = e

        if attempt < max_retries:
            sleep_time = base_sleep * (backoff ** (attempt - 1))
            sleep_time += random.uniform(0, 1.0)  # jitter
            print(
                f"Download failed for {tickers} on attempt {attempt}/{max_retries}. "
                f"Retrying in {sleep_time:.1f}s..."
            )
            time.sleep(sleep_time)

    print(f"Download failed for {tickers} after {max_retries} attempts: {last_error}")
    return pd.DataFrame()
...
def sort_by_growth(stock_list: list[str], day1: str, day2: str):
    ...
    all_dfs = download_with_retry(tickers=stock_list, start="1970-01-02", end=pd.Timestamp(day2)+pd.Timedelta(days=1), interval="1d")
...
def simulate_dual(sims: int, seed: int, stocks: list[str], begin_data_date: str, buy_date: str, sell1: str, sell2: str, analyze_node_features = False, frequently_appearing_check = False):
    ...
    stock_dfs = {}
    all_dfs = download_with_retry(tickers=stocks, start="1970-01-02", end=pd.Timestamp(sell2)+pd.Timedelta(days=1), interval="1d")
    for s in stocks:
    ...
    # Generate random sell dates using the SPX

    df = yf.download('^GSPC', start="1970-01-02", end=pd.Timestamp(sell2)+pd.Timedelta(days=1), interval="1d")
